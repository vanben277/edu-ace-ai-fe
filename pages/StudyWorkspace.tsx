...
const StudyWorkspace: React.FC = () => {
  ...
  return (
    <div
      className="flex h-[calc(100vh-64px)] overflow-hidden bg-slate-50 flex-col md:flex-row"
      style={{ cursor: isResizing ? "col-resize" : "default" }} // Hiện cursor kéo khi đang resize
    >
      <div className="absolute left-6 top-20 z-20 hidden lg:block">
        <Link
          to={isReadOnly ? "/admin/all-documents" : "/documents"}
          className="flex items-center gap-2 rounded-full bg-white px-4 py-2 text-xs font-bold text-slate-600 shadow-sm border border-slate-200 hover:bg-blue-600 hover:text-white transition-all"
        >
          <ArrowLeft size={14} />
      ...
      {/* --- THANH KÉO ĐIỀU CHỈNH (DRAG HANDLE) --- */}
      <div
        onMouseDown={startResizing}
        className={`w-1.5 bg-slate-200 hover:bg-blue-400 cursor-col-resize transition-colors flex items-center justify-center relative z-30 ${isResizing ? "bg-blue-500" : ""}`}
      >
        <div className="absolute h-10 w-4 bg-white border border-slate-200 rounded-full flex items-center justify-center shadow-sm">
          <GripVertical size={10} className="text-slate-400" />
        </div>
      </div>

      {/* VÙNG 2: SIDEBAR CHAT AI */}
      <div
        className="flex flex-col bg-white border-l border-slate-200 shadow-2xl relative transition-none"
        style={{ width: `${sidebarWidth}px`, minWidth: "300px" }} // Sử dụng style dynamic
      >
        {isReadOnly && (
          <div className="absolute inset-0 z-50 bg-white/80 backdrop-blur-[2px] flex flex-col items-center justify-center p-8 text-center">
            ...
            <p className="text-xs text-slate-500 mt-3 leading-relaxed font-bold">
              Chế độ quản trị viên không thể tương tác AI.
            </p>
          </div>
        )}
        ...
        <div className="flex h-16 items-center justify-between border-b px-6 bg-white/50 backdrop-blur-md">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-sm font-black text-slate-800 uppercase tracking-tighter line-clamp-1">
              Gia sư EduAce
            </h2>
          </div>
          {!isReadOnly && (
        ...
        <div
          ref={scrollRef}
          className="flex-1 space-y-6 overflow-y-auto p-6 bg-slate-50/30"
        >
          {!isReadOnly && messages.length === 0 && (
            <div className="text-center py-10">
              <p className="text-[11px] font-bold text-slate-400 uppercase tracking-widest">
                Bắt đầu hỏi AI về bài học
              </p>
            </div>
          )}
          ...
          {messages.map((msg, index) => (
            ...
            >
              <div
                className={`max-w-[90%] rounded-2xl px-4 py-3 text-[14px] leading-relaxed shadow-sm font-medium ${
                  msg.role === "user"
          ...
          {sending && (
            <div className="flex justify-start">
              <div className="flex items-center gap-2 bg-white border border-slate-100 rounded-2xl px-4 py-2 shadow-sm text-xs font-bold text-blue-600 animate-pulse">
                ...
                <span>EduAce đang phân tích...</span>
              </div>
            </div>
          )}
        </div>
          ...
          <div className="p-4 border-t bg-white">
            <form onSubmit={handleSend} className="relative">
              <input
                ...
                className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-4 pl-5 pr-14 text-sm font-bold outline-none focus:border-blue-500 focus:bg-white"
              />
              <button
                type="submit"
                disabled={sending || !input.trim()}
                className="absolute right-2 top-1/2 -translate-y-1/2 rounded-xl bg-blue-600 p-2.5 text-white shadow-lg"
              >
                <Send size={18} />
              </button>
            </form>
