...
import {
    CheckCircle, XCircle, ArrowLeft, Trophy, Sparkles, RefreshCw,
    GraduationCap, Info, Clock, ChevronRight, RotateCcw
} from "lucide-react";
                ...
                <>
                    ...
                    {loading ? (
                        ...
                        <div className="grid gap-4">
                            {history.map((item) => (
                                <div key={item.id} onClick={() => handleViewDetails(item.id)} className="bg-white p-6 rounded-2xl border border-slate-100 flex items-center justify-between cursor-pointer hover:shadow-lg transition-all group shadow-sm">
                                    <div className="flex items-center gap-6">
                                        <div className={`h-14 w-14 rounded-2xl flex items-center justify-center font-black text-lg ${item.score >= 5 ? 'bg-blue-50 text-blue-600' : 'bg-red-50 text-red-600'}`}>{item.score}</div>
                                        <div>
                                            <h3 className="font-bold text-slate-800 text-lg group-hover:text-blue-600 line-clamp-1">{item.quizTitle}</h3>
                                            <p className="text-[10px] text-slate-400 font-black uppercase">{item.correctAnswers}/{item.totalQuestions} câu đúng • {new Date(item.completedAt).toLocaleDateString('vi-VN')}</p>
                                        </div>
                                    </div>
                ...
                <div className="max-w-4xl mx-auto animate-in slide-in-from-bottom-4">
                    ...
                    {aiFeedback && (
                        <div className="bg-indigo-50 border-2 border-indigo-100 rounded-[2.5rem] p-8 prose prose-indigo max-w-none mb-10 shadow-sm animate-in zoom-in-95">
                            <h3 className="flex items-center gap-2 text-indigo-700 font-black mb-4"><GraduationCap /> Gia sư AI nhận xét:</h3>
                            <ReactMarkdown>{aiFeedback}</ReactMarkdown>
                        </div>
                    )}

                    <div className="space-y-10">
                        <h3 className="text-2xl font-black text-slate-800 ml-2">Phân tích chi tiết câu hỏi:</h3>

                        {(selectedResult.answers || []).map((answer: any, idx: number) => (
                            <div key={idx} className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm animate-in slide-in-from-bottom-2">

                                {/* 1. Nội dung câu hỏi và Số thứ tự */}
                                <div className="flex gap-5 mb-8">
                                    <div className={`h-11 w-11 min-w-[44px] flex items-center justify-center rounded-full text-white font-bold text-lg shadow-md ${answer.isCorrect ? 'bg-emerald-500' : 'bg-red-500'}`}>
                                        {idx + 1}
                                    </div>
                                ...
                                <div className="grid grid-cols-1 gap-4 mb-8">
                                    {['A', 'B', 'C', 'D'].map((label) => {
                                        ...
                                        return (
                                            <div key={label} className={`flex items-center justify-between p-5 rounded-[1.8rem] border-2 transition-all ${containerStyle}`}>
                                                <div className="flex items-center gap-4">
                                                    <span className={`h-9 w-9 min-w-[36px] flex items-center justify-center rounded-xl font-black border-2 
                        ${isCorrect ? 'border-emerald-300 bg-white' : isSelected ? 'border-red-300 bg-white' : 'border-slate-200 bg-white'}`}>
                                ...
                                <div className="bg-indigo-50/50 rounded-[2rem] p-7 border border-indigo-100 flex gap-5">
                                    ...
                                    <div>
                                        <p className="text-xs font-black text-indigo-600 uppercase tracking-[0.2em] mb-2">Gia sư AI hướng dẫn:</p>
                                        <p className="text-slate-700 leading-relaxed font-semibold text-[0.95rem]">
                                            {answer.explanation || "Không có giải thích chi tiết cho câu hỏi này."}
                                        </p>
                                    </div>
                                </div>
                            </div>
